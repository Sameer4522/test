...
import { errorToast, successToast } from 'src/api/utils'
import { AUTH } from 'src/api/apiServices'
  ...
  const submitForm = async () => {
    try {
      ...
      const res: any = await AUTH.login({
        username: formState.username.text,
      ...
      if (res.status === 200 || res.status === 202) {
        ...
        errorToast(res.message ? res.message : 'Something went wrong!')
      }
    } catch (error: any) {
        ...
        <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
          ...
          <form noValidate autoComplete='off' onSubmit={e => e.preventDefault()}>
            <TextField
              ...
              name='username'
              label='Username'
              error={formState.username.error.length > 0}
              value={formState.username.text}
              onChange={e => handleForm(e.target.name, e.target.value)}
              sx={!formState.username.error ? { marginBottom: 4 } : {}}
            />
            ...
            <Button fullWidth size='large' variant='contained' sx={{ marginBottom: 7 }} onClick={() => submitForm()}>
              Login
            </Button>
